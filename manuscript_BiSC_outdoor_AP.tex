...
We evaluated model performance using various statistical approaches and on different temporal resolutions. The LUR models were evaluated on an annual basis, DM was assessed both hourly and daily, and HM was evaluated weekly.
...
We assessed the performance of each hybrid model by contrasting predictions with observations. Two distinct validation approaches were employed. Firstly,  
\textit{10-fold cross-validation} was used to provide an internal validation measure to this purpose the model was trained using 80\% of the data available. Secondly, external validation was performed by allocating 20\% of the data as a test set to further evaluate the model's performance. While both methods vary in the sample size designated for training and testing, their core difference lies in the data-splitting strategy. In the 10-fold CV, the model trains on nine out of the ten data partitions and then validates against the remaining one, while in the external validation procedure the model uses all the data observations available to check the performance of the model.
...
The performance of the models varied substantially by pollutant \Cref{fig3}. Hybrid BC model had the best performance of the models assessed, with a higher R\textsuperscript{2} and lower RMSE. 
